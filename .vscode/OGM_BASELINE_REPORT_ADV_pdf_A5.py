...
# ==============================================================================
# VARNOSTNA FUNKCIJA: Prepreči, da bi prazni podatki izločili delnico
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            try:
                ...
                # Varno pridobivanje novih podatkov
                sektor = info_podatki.get('sector')
                if not sektor: sektor = 'Neznano'
            ...
            if is_falling_phase: status = "WARNING (FALLING PHASE)"
            elif not trend_ok: status = "WARNING (POD 200-MA)"
            elif trenutni_ogm >= 80.0: status = "STRONG BUY"
            ...
            # Shranjevanje podatkov za modalno okno
...
def generiraj_html_porocilo(podatki):
    ...
    <style>
    body {{ font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; background-color: #f8fafc; margin: 40px auto; max-width: 1400px; padding: 0 20px; }}
    ...
    <div class="header">
        ...
        <p id="navodilo-klik" style="font-size: 9pt; color: #93c5fd; margin-top: 5px;">* Sistem daje absolutno prednost nakupom vrhunskih podjetij v coni ugodnosti tik nad dolgoročnim 200w MA.</p>
        ...
        for _, row in df_html.iterrows():
            ...
            html_content += f"""
                <tr>
                    ...
                    <td><span class="{dist_class}">{row['Dist200w']:+.1f} %</span></td>
    ...
    function openAnalysis(ticker) {{
        ...
        let compHTML = `
            <table class="history-table">
                ...
                <tr><td>Dist do 200w MA</td><td>${{data.components.ma_dist_raw.toFixed(1)}} %</td><td style="font-weight:bold;">${{data.components.ma_dist_score.toFixed(1)}} / 40</td></tr>
