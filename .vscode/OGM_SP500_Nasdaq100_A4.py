...
.header {{ background: linear-gradient(135deg, #0c1628 0%, #112244 55%, #1d4ed8 100%); color: #fff; padding: 26px 30px; border-radius: 12px; border: 1px solid rgba(255,255,255,.07); box-shadow: 0 8px 40px rgba(0,0,0,.55); margin-bottom: 24px; position: relative; }}
.header h1 {{ margin: 0; font-size: 20pt; font-weight: 800; }}
.header p {{ margin: 5px 0 0; color: #93c5fd; font-size: 9.5pt; }}
.btn-tx {{ position: absolute; right: 28px; top: 50%; transform: translateY(-50%); background: #10b981; color: #fff; border: none; padding: 10px 18px; border-radius: 7px; font-weight: 700; cursor: pointer; box-shadow: 0 4px 14px rgba(16,185,129,.3); transition: .2s; }}
.btn-tx:hover {{ background: #059669; transform: translateY(calc(-50% - 1px)); }}
.stats-grid {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 22px; }}
.stat-card {{ background: #0b1526; padding: 20px; border-radius: 12px; border: 1px solid rgba(255,255,255,.06); text-align: center; }}
.stat-card h3 {{ margin: 0 0 8px 0; color: #475569; font-size: 10pt; text-transform: uppercase; letter-spacing: .5px; }}
.stat-card .val {{ font-size: 22pt; font-weight: bold; color: #f1f5f9; }}
.val-ogm {{ color: #4ade80; }}
.chart-container {{ background: #0b1526; padding: 18px; border-radius: 12px; border: 1px solid rgba(255,255,255,.06); height: 500px; margin-bottom: 22px; }}
table {{ width: 100%; border-collapse: collapse; background: transparent; border-radius: 8px; overflow: hidden; margin-bottom: 30px; }}
th {{ background: #09152a; color: #475569; padding: 12px 10px; text-align: center; border-bottom: 1px solid rgba(255,255,255,.06); font-size: 7.5pt; text-transform: uppercase; letter-spacing: .6px; font-weight: 700; }}
td {{ padding: 12px 10px; border-bottom: 1px solid rgba(255,255,255,.035); text-align: center; font-weight: bold; color: #94a3b8; }}
tr:nth-child(even) td {{ background: #0e1d32; }}
.pos {{ color: #4ade80; }} .neg {{ color: #f87171; }}
.modal {{ display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(0,0,0,.75); }}
.modal-content {{ background: #0b1526; border: 1px solid rgba(255,255,255,.08); margin: 3% auto; padding: 24px; border-radius: 12px; width: 80%; max-height: 85vh; overflow-y: auto; position: relative; }}
.close {{ color: #475569; position: absolute; right: 20px; top: 15px; font-size: 26px; font-weight: bold; cursor: pointer; }}
...
<div class="header">
    <h1>OGM Sklad &mdash; Backtest Poročilo</h1>
    <p style="margin:6px 0 0; color:#93c5fd; font-size:9.5pt;">2017–2024 &nbsp;|&nbsp; Začetni kapital: 100.000 € &nbsp;|&nbsp; Max 5% na delnico</p>
    <button class="btn-tx" onclick="openTx()">Transakcije</button>
</div>
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <div class="val" style="color:#94a3b8;">{benchmark_history['SP500'][-1]:,.2f} €</div>
    </div>
    <div class="stat-card">
        <h3>Končna vrednost NASDAQ 100</h3>
        <div class="val" style="color:#94a3b8;">{benchmark_history['NDX'][-1]:,.2f} €</div>
    </div>
</div>
...
<h2 style="color: #f1f5f9; font-size:14pt;">Primerjava Letnih Donosov (%)</h2>
<table>
...
<div id="txModal" class="modal">
    <div class="modal-content">
        ...
        <h2 style="margin-top:0; border-bottom: 1px solid rgba(255,255,255,.07); padding-bottom: 10px; color: #f1f5f9;">Zgodovina transakcij</h2>
        <table>
...
new Chart(ctx, {{
    ...
    options: {{ responsive: true, maintainAspectRatio: false, interaction: {{ mode: 'index', intersect: false }}, scales: {{ x: {{ ticks: {{ color: '#6b7280' }}, grid: {{ color: 'rgba(255,255,255,.05)' }} }}, y: {{ ticks: {{ color: '#6b7280' }}, grid: {{ color: 'rgba(255,255,255,.05)' }} }} }}, plugins: {{ legend: {{ labels: {{ color: '#94a3b8' }} }} }} }}
}});
