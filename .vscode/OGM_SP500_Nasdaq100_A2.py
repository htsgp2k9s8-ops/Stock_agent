...
print("=========================================================")
print("🚀 OGM PORTFOLIO BACKTESTER - Z MEGA-CAP PRAVILOM")
print("=========================================================")
...
MIN_MARKET_CAP = 40_000_000_000  # Osnovni pogoj 40 milijard
MEGA_CAP_THRESHOLD = 200_000_000_000 # 200 milijard za "Mag7" pravilo
...
# ==============================================================================
print("Kalkuliram OGM Buy in Sell točkovnike (z Mega-Cap pravilom)...")
signali = {}
...
for ticker in veljavni_tickerji.keys():
    ...
    if is_mega_cap:
        # MEGA-CAP PRAVILO: Uporabimo 50-week MA (~200-day MA) za iskanje vstopa
        ...
        # Ker je to hitrejši MA, dopustimo manjše odstopanje navzgor (max 20%)
        tocke_ma_buy[pos_mask] = np.interp(arr_pct[pos_mask], [0, 2, 10, 20], [40*(33/35), 40, 40*(15/35), 0.0])
        tocke_ma_buy[neg_mask] = np.interp(arr_pct[neg_mask], [-15, -5, 0], [0.0, 40*(25/35), 40*(33/35)])
    else:
        # KLASIČNO PRAVILO: Uporabimo 200-week MA za "Deep Value" vstop
...
<div class="header">
    ...
    <p>Začetni kapital: 100.000 € | Max 5% na delnico | Dinamičen filter: >200B Mega-Cap pravilo aktivirano</p>
...
function openTransactions() {{
    ...
    if (transactions.length === 0) {{
        ...
        transactions.forEach(tx => {{
            let colorClass = '';
            if (tx.Akcija.includes('MEGA-CAP')) colorClass = 'color:#d946ef; font-weight:bold;'; // Vijolična za Mag7
            else if (tx.Akcija.includes('BUY')) colorClass = 'color:#16a34a; font-weight:bold;'; // Zelena za klasične nakupe
            else colorClass = 'color:#dc2626; font-weight:bold;'; // Rdeča za prodaje
