...
def skeniraj_trg():
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            # Calculate MA and Volume for chart
...
def generiraj_html_porocilo(podatki):
    ...
    <div class="header">
        ...
        <p id="navodilo-klik" style="font-size: 9pt; color: #93c5fd; margin-top: 5px;">* Sistem uporablja 50w MA za Mega-Cap gigante, 200w MA (Moat) za High-Margin monopole in 200w MA za klasična podjetja. Klikni na ticker za analizo.</p>
    ...
    function openAnalysis(ticker) {{
        ...
        currentChart = new Chart(ctx, {{
            ...
            data: {{
                labels: data.dates,
                datasets: [
                    // Volume BUY (green bars)
                    {{ type: 'bar', label: 'Volume BUY', data: data.volume_buy || [], yAxisID: 'yVol', backgroundColor: 'rgba(34, 197, 94, 0.6)', borderColor: '#16a34a', borderWidth: 1, maxBarThickness: 6 }},
                    // Volume SELL (red bars)
                    {{ type: 'bar', label: 'Volume SELL', data: data.volume_sell || [], yAxisID: 'yVol', backgroundColor: 'rgba(239, 68, 68, 0.6)', borderColor: '#dc2626', borderWidth: 1, maxBarThickness: 6 }},
                    // Price (blue line)
                    {{ type: 'line', label: 'Cena ($)', data: data.prices, yAxisID: 'y', borderColor: '#2563eb', backgroundColor: 'rgba(37, 99, 235, 0.1)', borderWidth: 2, pointRadius: 0, tension: 0.1, fill: false }},
                    // 50w MA (orange dashed)
                    {{ type: 'line', label: '50w MA', data: data.ma50 || [], yAxisID: 'y', borderColor: '#f97316', borderWidth: 1.5, pointRadius: 0, borderDash: [4, 4], fill: false }},
                    // 200w MA (green dashed)
                    {{ type: 'line', label: '200w MA', data: data.ma200 || [], yAxisID: 'y', borderColor: '#10b981', borderWidth: 1.5, pointRadius: 0, borderDash: [2, 4], fill: false }},
                    // OGM Score
                    {{ type: 'line', label: 'OGM Score', data: data.ogm, yAxisID: 'y1', borderColor: '#16a34a', borderWidth: 1.5, pointRadius: 0, tension: 0.3, borderDash: [5, 5], fill: false }}
                ]
            }},
            options: {{
                ...
                yVol: {{ type: 'linear', display: true, position: 'right', title: {{ display: true, text: 'Volumen' }}, grid: {{ drawOnChartArea: false }}, suggestedMin: 0 }}
        }},
        plugins: {{
            legend: {{ position: 'top' }},
            zoom: {{
