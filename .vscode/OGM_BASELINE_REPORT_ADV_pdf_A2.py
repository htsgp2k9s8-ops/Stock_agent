...
MIN_REVENUE_GROWTH = 0.10            # Minimalno 10 % pričakovana rast prihodkov
SUPER_GROWTH_THRESHOLD = 0.50        # Izpostavljena super-rast > 50 % (ne glede na OGM)
...
def skeniraj_trg():
    ...
    print(f"        -> Kriterij 1: Market Cap > {MIN_MARKET_CAP/1e9:.0f}B (Brez N/A)")
    print(f"        -> Kriterij 2: Ocenjena rast prihodkov > {MIN_REVENUE_GROWTH*100:.0f}%")
    print(f"        -> Kriterij 3: Zaščita pred padanjem (50-day MA vs 200-week MA)")
    print("---------------------------------------------------------")
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            # --- INFO: TRŽNA VREDNOST, RAST PRIHODKOV IN IME ---
            try:
            ...
            # FILTER 1: Če Market Cap ne obstaja ali je pod 20 milijardami, preskočimo
            ...
            # Prenos podatkov (samo varen in zanesljiv TEDENSKI interval!)
            ...
            # 10 tednov je točno 50 trgovalnih dni. Tako se rešiva dnevnega klica in napak!
            ma50 = close_series.rolling(window=10).mean()
            
            zadnji_ma200_weekly = float(ma200.iloc[-1])
            zadnji_ma50_weekly = float(ma50.iloc[-1])
            
            # Varnostna blokada za ZeroDivisionError
            ...
            # Indikator faze padanja (kratkoročni trend pod dolgoročnim)
            is_falling_phase = zadnji_ma50_weekly < zadnji_ma200_weekly
            razmerje_trend = zadnji_ma50_weekly / zadnji_ma200_weekly
            ...
            # --- STRATEŠKA KATEGORIZACIJA ---
            if rev_growth_val >= SUPER_GROWTH_THRESHOLD:
                status = "SUPER-GROWTH TARGET"
            elif is_falling_phase:
                status = "WARNING (FALLING PHASE)"
            elif not trend_ok:
                status = "WARNING (POD 200-MA)"
            elif rev_growth_val >= MIN_REVENUE_GROWTH:
                if trenutni_ogm >= 80.0: status = "STRONG BUY"
                elif trenutni_ogm >= 65.0: status = "BUY"
                else: continue
            else:
                continue 
            
            # --- ZGODOVINSKI BACKTEST ZA SEZNAM UNIKATNIH SIGNALOV ---
            datumi_str = [d.strftime("%Y-%m-%d") for d in close_series.index]
            zgodovina_tabela = []
            
            if status in ["STRONG BUY", "BUY", "WARNING (FALLING PHASE)", "SUPER-GROWTH TARGET"]:
            ...
            rezultati.append({
                ...
                "Ratio50_200": float(razmerje_trend)
            })
...
def generiraj_html_porocilo(podatki):
    ...
    if not df_html.empty:
        status_order = {"SUPER-GROWTH TARGET": 0, "STRONG BUY": 1, "BUY": 2, "WARNING (FALLING PHASE)": 3, "WARNING (POD 200-MA)": 4}
        df_html['Order'] = df_html['Status'].map(status_order)
        df_html = df_html.sort_values(by=["Order", "OGM"], ascending=[True, False])
    ...
    <div class="header">
        ...
        <p>Filtri: Market Cap > 20B | Ocenjena rast prihodkov > 10% | Osveženo: {datum_danes}</p>
        <p id="navodilo-klik" style="font-size: 9pt; color: #93c5fd; margin-top: 5px;">* Klikni na tiker delnice za grafično analizo in 10-letni zgodovinski backtest.</p>
        ...
        for _, row in df_html.iterrows():
            ...
            html_content += f"""
                <tr>
                    ...
                    <td style="font-weight:600; color:#475569;">{row['Ratio50_200']:.2f}x</td>
    ...
    function openAnalysis(ticker) {{
        ...
        // Čisti JS template literali brez napačnih poševnic
