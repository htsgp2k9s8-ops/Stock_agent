...
def get_historical_price(ticker, date_str):
    """Return (price, actual_date) at or just after date_str."""
    try:
...
def get_live_price(ticker):
    """Return (price, info_dict) using today's market data."""
    try:
...
print("  3  Osveži porocilo  (brez sprememb)")
print("  4  Ponastavi portfolio  (nazaj na $50k)")
print()

choice = input("Izbira (1/2/3/4): ").strip()
                ...
                try:
                    if is_retroactive:
                        ...
                        if price is None:
                            ...
                            # Fetch company meta separately (live is fine for display only)
                            try:
                                info = yf.Ticker(ticker).info
                            except Exception:
                                info = {}
        ...
        if ticker not in portfolio["positions"]:
            ...
            if not price or price <= 0:
                ...
                if shares_to_sell <= 0:
                    ...
                    # FIFO deduction across lots (oldest first)
                    ...
                    for lot in sorted(lots, key=lambda x: x["open_date"]):
                        ...
                        if lot["shares"] <= remaining + 1e-8:
                            ...
                            # Partial lot: deduct proportionally
...
# ═══════════════════════════════════════════════════════════════════════════════
# REPORT GENERATION — only on explicit choice 3
# ═══════════════════════════════════════════════════════════════════════════════
if choice != "3":
    print("\n[OK] Akcija shranjena. Zaženi skript znova za naslednji nakup/prodajo, ali izberi 3 za porocilo.")
    import sys; sys.exit(0)

print("\nPridobivam tržne podatke za porocilo ...")

# Determine chart start: earliest lot open_date (or portfolio created date)
...
# Chart goes back at least 1 year for context when portfolio is very new
...
# ── Current stats per ticker (aggregated across lots) ────────────────────────
holdings = []
today_ts = date.today()
...
# ── Time-series: portfolio reconstructed from each lot's open_date ────────────
chart_labels    = []
chart_portfolio = []
chart_sp500     = []
chart_ndx       = []
...
if not price_data.empty:
    # Benchmark base: first available row at/after chart_start
    ...
    if not base_candidates.empty:
        ...
        for dt in price_data.index:
            ...
            # Reconstruct portfolio value: only count lots that were opened on/before dt
...
# Current return vs benchmark (from chart_start = same $50k base)
