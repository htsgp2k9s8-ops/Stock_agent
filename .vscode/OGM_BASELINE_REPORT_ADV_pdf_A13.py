...
.header {{ background: linear-gradient(135deg, #0f172a 0%, #1d4ed8 100%); color: #fff; padding: 30px; border-radius: 8px; border-bottom: 4px solid #2563eb; position: relative; }}
.header h1 {{ margin: 0; font-size: 22pt; font-weight: 700; }}
.header p {{ margin: 5px 0 0 0; color: #bfdbfe; font-size: 11pt; }}
.btn-pdf {{ position: absolute; right: 30px; top: 35px; background-color: #ef4444; color: #fff; border: none; padding: 12px 20px; border-radius: 6px; font-weight: 700; font-size: 10pt; cursor: pointer; box-shadow: 0 4px 6px rgba(0,0,0,.1); transition: .2s; }}
.btn-pdf:hover {{ background-color: #dc2626; transform: translateY(-1px); }}
table {{ width: 100%; border-collapse: collapse; background-color: #fff; border: 1px solid #e2e8f0; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,.05); margin-top: 25px; table-layout: auto; }}
th {{ background-color: #1e293b; color: #fff; text-align: left; padding: 12px 10px; font-size: 9.5pt; font-weight: 600; }}
...
.clickable-ticker {{ cursor: pointer; color: #1d4ed8; font-weight: 700; transition: color .15s; }}
.clickable-ticker:hover {{ color: #1e40af; text-decoration: underline; }}
...
/* Dark table */
.dark-tbl {{ width: 100%; border-collapse: collapse; font-size: 9.5pt; }}
.dark-tbl th {{ background: #111d33; color: #475569; padding: 10px 14px; text-align: left; font-size: 8pt; text-transform: uppercase; letter-spacing: .5px; border-bottom: 1px solid #1e293b; }}
.dark-tbl td {{ padding: 10px 14px; border-bottom: 1px solid #0d1526; color: #cbd5e1; }}
.dark-tbl tr:hover td {{ background: #111d33; }}
...
<div class="header">
    ...
    <p id="navodilo-klik" style="font-size:9pt; color:#93c5fd; margin-top:5px;">* Sistem uporablja 50w MA za Mega-Cap gigante, 200w MA (Moat) za High-Margin monopole in 200w MA za klasična podjetja. Klikni na ticker za podrobno analizo.</p>
...
"""
    ...
    if not df_html.empty:
        for rank, (_, row) in enumerate(df_html.iterrows(), 1):
            ...
            ticker_display = f'<span class="clickable-ticker" onclick="openAnalysis(\'{row["Ticker"]}\', {rank})">&#x1F4CA; {row["Ticker"]}</span>'

            tp_display = f"${row['TargetPrice']:.2f}" if row['TargetPrice'] > 0 else "N/A"

            html_content += f"""
            <tr>
                <td style="color:#94a3b8; font-weight:700; font-size:10pt;">#{rank}</td>
...
<div id="analysisModal" class="modal">
  <div class="modal-content">
    ...
    <div class="modal-tab-bar">
      ...
      <button class="tab-btn tab-soon" onclick="switchTab(this,'tab-soon')">Ratings</button>
    ...
    <div class="modal-body">
      ...
      <div class="tab-pane active" id="tab-summary">
        <div class="tab-section-lbl">Graf cene &amp; OGM Score (tedenski)</div>
        <div class="chart-container"><canvas id="ogmChart"></canvas></div>
      </div>
...
function buildSignals(data) {{
    ...
    if (!data.history || data.history.length === 0) {{
        tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:#334155;padding:30px;">Ni zabeleženih preteklih signalov.</td></tr>';
        return;
    }}
    data.history.forEach(function(row) {{
        const color = row.max_donos >= 0 ? '#4ade80' : '#f87171';
        const tr = document.createElement('tr');
        tr.innerHTML = '<td>' + row.datum + '</td><td>$' + row.cena.toFixed(2) + '</td><td>$' + row.max_cena.toFixed(2) + '</td><td style="color:' + color + ';font-weight:700;">+' + row.max_donos.toFixed(1) + '%</td>';
