...
# ==============================================================================
def skeniraj_trg():
    print(f"\n[START] Začenjam skeniranje za {len(tickerji_v_obdelavi)} operativnih delnic...")
    print(f"        (Aktiviran filter: Samo podjetja nad {MIN_MARKET_CAP/1e9:.1f} milijarde $)")
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            try:
                ...
                mcap = info_podatki.get('marketCap', 0)
...
# ==============================================================================
# 3. GENERIRANJE HTML POROČILA S PDF IZVOZOM
# ==============================================================================
def generiraj_html_porocilo(podatki):
    ...
    if not df_html.empty:
        ...
        df_html = df_html.sort_values(by=["OGM", "Mcap_Raw"], ascending=[False, False])
    ...
    .btn-pdf:hover {{ background-color: #dc2626; transform: translateY(-1px); box-shadow: 0 6px 8px rgba(0,0,0,0.15); }}
    ...
    /* OPTIMIZACIJA ZA IZVOZ V PDF */
    @media print {{
        /* Prisilimo brskalnik, da natisne barvna ozadja in značke */
        body {{ background-color: white; margin: 0; padding: 0; -webkit-print-color-adjust: exact; print-color-adjust: exact; }}
        /* Skrijemo gumb in navodila za klikanje, saj na papirju nimajo smisla */
    ...
    <div class="header">
        ...
        <p>Prečiščeni rezultati (Market Cap Filter: > ${MIN_MARKET_CAP/1e9:.1f}B) | Osveženo: {datum_danes}</p>
        <p id="navodilo-klik" style="font-size: 9pt; color: #93c5fd; margin-top: 5px;">* Klikni na tiker pri STRONG BUY za grafično analizo in 10-letni backtest.</p>
    ...
    if not df_html.empty:
        ...
        html_content += """<tr><td colspan="8" style="text-align:center;">Trenutno ni delnic v nakupnem območju, ki ustrezajo filtrom.</td></tr>"""
