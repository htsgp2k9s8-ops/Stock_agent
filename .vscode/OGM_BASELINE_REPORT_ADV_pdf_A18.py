...
# ── CLI date argument: python script.py 2024-06-01 ──────────────────────────
SCAN_DATE = None
if len(sys.argv) > 1 and os.environ.get("CI") != "true":
    try:
        SCAN_DATE = datetime.strptime(sys.argv[1], "%Y-%m-%d").date()
        print(f"[INFO] HISTORIČNO SKENIRANJE za datum: {SCAN_DATE}")
    except ValueError:
        print(f"[OPOZORILO] Neveljaven datum '{sys.argv[1]}'. Pričakovan format: YYYY-MM-DD. Skeniranje za danes.")
...
def berem_sektor_podatke(scan_date=None):
    ...
    try:
        if scan_date:
            ...
            raw_data = yf.download(etfs, start="2020-01-01", end=end_dt, interval="1wk", progress=False)
        else:
...
def skeniraj_trg():
    ...
    trenutno_leto = SCAN_DATE.year if SCAN_DATE else datetime.now().year
    rezultati = []
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            data = delnica.history(start="2010-01-01", end=dl_end, interval="1wk", actions=False)
            ...
            if trenutni_ogm < 65.0:
                continue
            if not SCAN_DATE and rev_growth_val < MIN_REVENUE_GROWTH:
                continue
    ...
    sektor_primerjava = berem_sektor_podatke(scan_date=SCAN_DATE)
...
def generiraj_html_porocilo(podatki, sektor_primerjava=None):
    ...
    if SCAN_DATE:
        datum_danes = f"HISTORIČNI POGLED — {SCAN_DATE.strftime('%d.%m.%Y')}  (fundamentals: trenutni)"
    else:
...
"""
    ...
    ime_datoteke = f"ogm_{SCAN_DATE}.html" if SCAN_DATE else "index.html"
