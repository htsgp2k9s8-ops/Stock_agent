...
print("=========================================================")
print("🎯 OGM WATCHLIST RADAR: VEČKRATNA DIAGNOSTIKA (MOAT V3)")
print("=========================================================")
...
def zagon_watchlist_skenerja():
    ...
    print("        -> V3 Mega-Cap in High-Margin Moat pravila AKTIVIRANA.")
    print("---------------------------------------------------------")
    ...
    for ticker in tickerji:
        ...
        try:
            ...
            try:
                ...
                if target_price == 0.0: target_price = safe_float(info_podatki.get('targetMedianPrice'))
                    
                pe_ratio = safe_float(info_podatki.get('trailingPE'))
                if pe_ratio == 0.0: pe_ratio = safe_float(info_podatki.get('forwardPE'))
            ...
            if razlogi and data is None: continue
                
            if isinstance(data.columns, pd.MultiIndex):
                data.columns = data.columns.get_level_values(0)
                
            close_series = data['Close'].dropna()
            if close_series.empty or len(close_series) < 205: continue
            ...
            if trenutni_ogm < 65.0: razlogi.append(f"OGM Score je prenizek ({trenutni_ogm:.1f} < 65).")
    ...
    if rezultati: generiraj_html_porocilo(rezultati, graf_podatki_vsi)
    else: print("Ni bilo uspešno prenesenih nobenih delnic.")
...
def generiraj_html_porocilo(podatki, graf_podatki_vsi):
    ...
    .card-header {{ display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,.07); padding-bottom: 14px; margin-bottom: 18px; }}
    .card-title {{ font-size: 15pt; font-weight: bold; color: #f1f5f9; margin: 0; }}
    .card-subtitle {{ font-size: 9.5pt; color: #475569; margin-top: 4px; }}
    .status-badge {{ padding: 5px 14px; border-radius: 20px; font-weight: 700; font-size: 9pt; color: #fff; display: inline-block; }}
    .grid-metrics {{ display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; margin-bottom: 18px; }}
    .metric-box {{ background: #0e1d32; padding: 14px; border-radius: 8px; border: 1px solid rgba(255,255,255,.05); text-align: center; }}
    .metric-box.highlight {{ background: #0c1a3a; border-color: rgba(59,130,246,.2); }}
    .metric-label {{ font-size: 8pt; color: #475569; text-transform: uppercase; letter-spacing: .6px; margin-bottom: 6px; }}
    .metric-val {{ font-size: 15pt; font-weight: bold; color: #f1f5f9; }}
    .val-green {{ color: #4ade80; }}
    .val-red {{ color: #f87171; }}
    .filter-box {{ background: #0e1d32; padding: 14px; border-radius: 8px; margin-bottom: 18px; border-left: 3px solid #334155; }}
    .filter-box h4 {{ margin: 0 0 8px 0; color: #64748b; font-size: 9pt; text-transform: uppercase; letter-spacing: .5px; }}
    .filter-box ul {{ margin: 0; padding-left: 18px; font-size: 9pt; color: #64748b; }}
    .chart-container {{ position: relative; height: 350px; width: 100%; margin-bottom: 22px; }}
    ...
    @media print {{
        ...
        .card {{ box-shadow: none; page-break-inside: avoid; margin-bottom: 15px; }}
...
<div class="page-wrap">
    ...
    <div class="header">
        <h1>OGM Watchlist Radar</h1>
        <p style="margin:6px 0 0; color:#93c5fd; font-size:9.5pt;">Generirano: {datum_danes}</p>
    </div>
    """
        ...
        <div class="card">
            ...
            <div class="grid-metrics">
                <div class="metric-box highlight">
                    ...
                    <div class="metric-val" style="color: #60a5fa;">{row['OGM']:.1f} / 100</div>
                </div>
    ...
    Object.keys(chartData).forEach(ticker => {
        ...
        let compHTML = `
            ...
            <tr style="background: #0e1d32; font-weight: bold; font-size: 10.5pt; color: #f1f5f9;"><td>SKUPAJ OGM</td><td>-</td><td>${data.components.total_score.toFixed(1)} / 100</td></tr>
        `;
