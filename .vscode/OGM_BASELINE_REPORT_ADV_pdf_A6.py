...
# ==============================================================================
# STRATEŠKI INSTITUCIONALNI FILTRI (Market Cap je strogo samo začetni filter!)
# ==============================================================================
...
def skeniraj_trg():
    ...
    print(f"        -> Začetni filter: Velikost podjetja (Market Cap) > {MIN_MARKET_CAP/1e9:.0f}B")
    print(f"        -> Začetni filter: Ocenjena rast prihodkov > {MIN_REVENUE_GROWTH*100:.0f}%")
    print(f"        -> OGM Točkovnik: 200w MA je KRALJ tajminga (Maksimalno 40 točk od 100)")
    print("---------------------------------------------------------")
    ...
    for krog, ticker in enumerate(tickerji_v_obdelavi, 1):
        ...
        try:
            ...
            try:
                ...
                sektor = info_podatki.get('sector')
                if not sektor:
                    sektor = "Neznano"
            ...
            # ZAČETNI FILTER ELIMINACIJE: Če ne ustreza velikosti, skočimo iz zanke
            ...
            # ==================================================================
            # ŽIVA FUNDAMENTALNA INTERPOLACIJA (Po tvoji formuli)
            # ==================================================================
            # 1. Pridobivanje trenutnih podatkov iz Yahoo Finance
            eps_growth = info_podatki.get('earningsQuarterlyGrowth', 0) or info_podatki.get('earningsGrowth', 0)
            peg_ratio = info_podatki.get('pegRatio', 2.0) # Če podatka ni, predpostavimo slabo vrednost
            gross_margin = info_podatki.get('grossMargins', 0)
            
            # Pretvorba v procente za formulo
            eps_growth_pct = eps_growth * 100 if eps_growth else 0.0
            gross_margin_pct = gross_margin * 100 if gross_margin else 0.0
            peg_ratio_val = float(peg_ratio) if peg_ratio else 2.0
            
            # 2. Interpolacija točno po tvoji sliki
            ...
            # 3. Seštevek z omejitvijo na 45 točk (krovna meja za fundamente)
            base_fundament_score = min(45.0, tocke_eps + tocke_peg + tocke_margin)
                
            # ==================================================================
            # TEHNIČNI TOČKOVNIK (PREOSTALIH 55 TOČK - 200w MA JE KRALJ)
            # ==================================================================
            ...
            # Razporeditev: MA200 prinaša do 40 točk, RSI do 15 točk. Skupaj s 45 osnovo = 100 točk.
            W_MA = 40.0  
            W_RSI = 15.0 
            
            # Generiranje natančne krivulje interpolacije, prilagojene na 40 točk maksimuma
            ...
            # Skupni OGM Faktor (Strogo omejen na 100 točk)
            ...
            # Izločimo delnice pod pragom 65 ali tiste, ki ne prebijejo rasti prihodkov
            if trenutni_ogm < 65.0 or rev_growth_val < MIN_REVENUE_GROWTH:
                continue 
            close_series.index = pd.to_datetime(close_series.index)
            monthly_prices = close_series.groupby(
                [close_series.index.year, close_series.index.month]
            ).last()
            monthly_returns = monthly_prices.pct_change() * 100
            yearly_prices = close_series.groupby(
                close_series.index.year
            ).last()
            ...
            yearly_data = [
                ...
                for y, val in yearly_returns.dropna().items()
            ]
            monthly_data = {}
            ...
            strong_buy_podatki_za_graf[ticker] = {
                ...
                "components": {
                    "eps_growth_raw": eps_growth_pct,
                    "eps_growth_score": float(tocke_eps),
                    "peg_raw": peg_ratio_val,
                    "peg_score": float(tocke_peg),

                    "pe_raw": pe_ratio,

                    "margin_raw": gross_margin_pct,
                    "margin_score": float(tocke_margin),

                    "rsi_raw": float(rsi_arr[-1]),
                    "rsi_score": float(tocke_rsi[-1]),

                    "ma_dist_raw": oddaljenost_ma_trenutna,
                    "ma_dist_score": float(tocke_ma[-1]),
...
def generiraj_html_porocilo(podatki):
    ...
    <div class="header">
        ...
        <p>Začetni filtri: Market Cap > 20B | Rast prihodkov > 10% | OGM Točkovnik: 200w MA je kralj tajminga | Osveženo: {datum_danes}</p>
        <p id="navodilo-klik" style="font-size: 9pt; color: #93c5fd; margin-top: 5px;">* Sistem daje prednost varnim vstopom ob dotiku dolgoročnega 200-week MA drsečega povprečja.</p>
        ...
        for _, row in df_html.iterrows():
            ...
            html_content += f"""
                <tr>
                    ...
                    <td style="color:#64748b;">{row['Sektor']}</td>
                </tr>
            """
    else:
        html_content += """<tr><td colspan="10" style="text-align:center;">Trenutno ni delnic, ki ustrezajo kriterijem.</td></tr>"""
